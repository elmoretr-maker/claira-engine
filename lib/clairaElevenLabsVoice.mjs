...
/** Default Claira voice when `ELEVENLABS_VOICE_ID` is unset. */
...
/**
 * Resolved ElevenLabs voice id: `ELEVENLABS_VOICE_ID` env, else {@link CLAIRA_ELEVENLABS_VOICE_ID}.
 * @returns {string}
 */
export function getResolvedElevenLabsVoiceId() {
  const fromEnv = typeof process !== "undefined" ? process.env.ELEVENLABS_VOICE_ID?.trim() : "";
  return fromEnv || CLAIRA_ELEVENLABS_VOICE_ID;
}

/**
 * @param {string} [explicitVoiceId]
 * @returns {string}
 */
export function resolveClairaVoiceId(explicitVoiceId) {
  if (typeof explicitVoiceId === "string" && explicitVoiceId.trim()) return explicitVoiceId.trim();
  return getResolvedElevenLabsVoiceId();
}
...
export async function synthesizeClairaSpeech(text, options = {}) {
  ...
  const voiceId = resolveClairaVoiceId();
