...
export const folderStructureModule = {
  ...
  run(input, context) {
    ...
    if (getDomainDefinition(dm).id === "contractor") {
      ...
      const root =
        String(input.rootFolder ?? "Projects").replace(/\\/g, "/").replace(/^\/+|\/+$/g, "") || "Projects";
      const suggestedFolderPath = `${root}/${hints.projectSlug}/Rooms/${hints.roomSlug}/Timeline/${hints.stageSlug}`;
      return {
        suggestedFolderPath,
        summary: `Would place under ${suggestedFolderPath}/ (dry-run).`,
      };
