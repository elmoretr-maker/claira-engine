...
/** Build a default dataset name from intent + current month/year. */
function buildDefaultName(intent) {
  const labels = getLabels(intent);
  const d      = new Date();
  const month  = d.toLocaleString("default", { month: "short" });
  const year   = d.getFullYear();
  return `${labels.intentLabel} — ${month} ${year}`;
}
 ...
 * }} props
 */
export default function BusinessAnalyzerFlow({
  mode            = "create",
